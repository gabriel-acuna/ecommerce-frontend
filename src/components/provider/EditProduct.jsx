...
import { getProduct, updateProduct } from '../../services/productos.service';
...
export default (props) => { 
    ...
    useEffect(() => {
        ...
        if (Object.keys(authData).length === 0) {
            ...
            getProduct(id).then(
                r=> {
                    setMarca(r.modelo.marca.id);
                    cargarModelos(r.modelo.marca.id);
                    setModel(r.modelo.id);
                    setDescription(r.descripcion);
                    setPrice(r.precio);
                    setUrlImage(r.urlImagen);
                }
            );
        } else {
            history.push("/");
    ...
    let sendData = async (event) => {
        ...
        let resp = await updateProduct(id,{
            model,
            provider,
            price,
            description,
                        ...
                        <div className="field" style={{ padding: "20px 100px" }}>
                            <form onSubmit={event => sendData(event)}>
                                ...
                                <div className="select">
                                    <select name="brand" id="#brand" onChange={event => { setMarca(event.target.value);cargarModelos(event.target.value) }} required value={marca}>
                                ...
                                <div className="control mt-4">
                                    <img src={urlImage} alt="product" style={{with:'120px'}}/>
                                </div>
